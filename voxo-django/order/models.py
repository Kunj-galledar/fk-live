...
def create_unique_transaction_id(length=15):
    not_unique = True
    while not_unique:
        s = ''
        for i in range(length):
            s += random.choice(digits + ascii_uppercase)
        if not OrderPayment.objects.filter(orderPaymentTransactionId=s):
            not_unique = False
    return s
...
class Cart(models.Model):
    ...
    # cartTotalProduct = models.PositiveIntegerField(
...
class CartProducts(models.Model):
    ...
    cartByCustomer = models.ForeignKey(CustomUser, on_delete=models.CASCADE,null=True, verbose_name='Customer')
    cartProduct = models.ForeignKey(ProductVariant, on_delete=models.CASCADE, blank=True, null=True, verbose_name='Product')
    cartProductQuantity = models.PositiveIntegerField(verbose_name='Quantity',null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class OrderPayment(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    # orderPaymentFromOrder=models.ForeignKey(Order,on_delete=models.CASCADE,verbose_name='Paymnt of Order')
    orderPaymentFromCustomer=models.ForeignKey(CustomUser,on_delete=models.CASCADE,verbose_name='Customer',related_name='opfc')
    # orderPaymentToAdmin=models.ForeignKey(CustomUser,on_delete=models.CASCADE,verbose_name='Payment to Admin',related_name='opta')
    ...
    # orderNameOnCard=models.CharField(max_length=20,verbose_name='Name on Card')
    # orderCardNumber=models.CharField(max_length=20,verbose_name='Card Number')
    # orderExpireationDate=models.CharField(max_length=20,verbose_name='Card Expairation Date')
    # orderCVV=models.CharField(max_length=20,verbose_name='Card CVV')
...
class Order(models.Model):
    ...
    # orderCoupon=models.ForeignKey(Coupon,on_delete=models.CASCADE,blank=True,verbose_name='Coupon')
...
class OrderTracking(models.Model):
    ...
    # trackingOrder = models.ForeignKey(ProductVariant, on_delete=models.CASCADE, verbose_name='Product')
    ...
    # trackingOrderStatus = StatusField()
