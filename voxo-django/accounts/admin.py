...
from voxo.settings import MESSAGE_TAGS
...
class AdminAdmin(admin.ModelAdmin):
    list_display = ('adminName','admin','adminContact','adminWalletBalance','minimumWithdrawalAmount','created_at')

@admin.register(Vendor)
class VendorAdmin(admin.ModelAdmin):
    list_display = ('vendorName','vendor','vendorContact','vendorGst',
                    'bankName','bankAccountName','bankAccountNumber','bankIFSCCode','bankSWIFTCode',
                    'razorPayEmailId','payPalEmailId',
                    'vendorWalletBalance','created_at')
    
    def get_queryset(self, request):

        if request.user.is_vendor:
            queryset = super(VendorAdmin, self).get_queryset(request)
            return queryset.filter(vendor=request.user)
        else:
...
# ================================================================================================= 
    ...
    #     return ['vendorWalletBalance']
        

#     def get_form(self, request, obj=None, **kwargs):
#         kwargs['widgets'] = {
#             'vendorName': forms.TextInput(attrs={'placeholder': 'Enter Full Name'}),
#             'vendorContact': forms.TextInput(attrs={'placeholder': 'Enter Contact Number'}),
#             'vendorGst': forms.TextInput(attrs={'placeholder': 'Enter Contact Number'}),
#         }
#         return super().get_form(request, obj, **kwargs)
...
#         # Custom logic for your view
#         # You can perform additional queries or data manipulation here
        
#         # Render your own template
#         # context = {'custom_data': 'Custom data to pass to the template'}
#         context = {
#             'custom_data': 'Custom data to pass to the template',
#             'opts': self.model._meta,  # Include the model's meta information
#             'app_label': self.model._meta.app_label  # Include the app_label
#         }
#         return render(request, 'admin/accounts/vendor/custom_changelist.html', context)
...
#         # Render your own template
#         context = {'custom_data': 'Custom data to pass to the template'}
#         return render(request, 'admin/myapp/mymodel/custom_changelist.html', context)